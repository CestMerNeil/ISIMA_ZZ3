...
int main(int argc, char **argv)
{
	...
	XSetStandardProperties(display, racine, tit_racine, tit_racine, None, argv, argc, &indic);
	...
	while (encore) {
		...
		switch (ev.type) {
			case Expose :
				...
				/* ev.xexpose.width et ev.xexpose.height sont les dimensions de la zone a rafraichir */
				if (ev.xexpose.count == 0) {
					...
					for (xe=0; xe<indic.width; xe++)
						for (ye=0; ye<indic.height; ye++) {
							...
							else c = couleur3(x); /* couleur1(x)%3; */
