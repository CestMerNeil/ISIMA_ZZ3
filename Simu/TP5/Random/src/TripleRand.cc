...
namespace CLHEP
{
  ...
  void TripleRand::showStatus() const
  {
    ...
    std::cout << "Hurd288Engine generator= " << std::endl
              << ConstHurd();
    std::cout << std::endl
              << "-----------------------------------------"
              << std::endl;
  ...
  {
    return (float)((integerCong ^ tausworthe ^ static_cast<unsigned int>(hurd)) * twoToMinus_32 + nearlyTwoToMinus_54);
    // make sure non-zero!
  }

  TripleRand::operator unsigned int()
  {
    return integerCong ^ tausworthe ^ static_cast<unsigned int>(hurd);
  }
  ...
  bool TripleRand::getState(const std::vector<unsigned long> &v)
  {
    ...
    if (!tausworthe.get(iv))
      return false;
    if (!integerCong.get(iv))
      return false;
